...
try:
    ...
    # ✅ تحميل نموذج العلامات بشكل آمن
    predictor_path = "shape_predictor_68_face_landmarks.dat"
    if os.path.exists(predictor_path):
        predictor = dlib.shape_predictor(predictor_path)
        logger.info("✓ تم تحميل نموذج العلامات")
    else:
        logger.warning(f"⚠️ لم يتم العثور على {predictor_path}")
        predictor = None
        DLIB_AVAILABLE = False
except ImportError:
    DLIB_AVAILABLE = False
    logger.warning("⚠️ dlib غير مثبت - سيتم استخدام طريقة بديلة")
    detector = None
    predictor = None
except Exception as e:
    DLIB_AVAILABLE = False
    logger.error(f"خطأ في تحميل dlib: {str(e)}")
    detector = None
    predictor = None
...
def check_eye_openness(image, ear_threshold=0.2):
    ...
    try:
        if not DLIB_AVAILABLE or detector is None or predictor is None:
            logger.warning("⚠️ dlib غير متاح - تخطي فحص العينين")
            return True
        ...
        # ✅ التحقق من توفر predictor قبل الاستخدام
        if predictor is None:
            logger.warning("⚠️ predictor غير متاح")
            return True
        ...
        logger.info(f"👁️ حالة العينين: {'✓ مفتوحة' if is_open else '✗ مغلقة'}")
...
def detect_blinking(image_sequence, ear_threshold=0.2, consecutive_frames=3):
    ...
    try:
        if not DLIB_AVAILABLE or detector is None or predictor is None:
            logger.warning("⚠️ dlib غير متاح - تخطي كشف الرمش")
            return True
        ...
        for image in image_sequence:
            try:
                ...
                logger.warning(f"⚠️ خطأ في معالجة إطار: {str(e)}")
                continue
